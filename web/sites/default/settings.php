...
// If a dotenv file exists outside of Drupal root, load it.
...
if(file_exists(DRUPAL_ROOT . '/../salt.txt')) {
  $settings['hash_salt'] = trim(file_get_contents(DRUPAL_ROOT . '/../salt.txt'));
} elseif(getenv('CI') == 'true') {
  // We running under codeship
  $settings['hash_salt'] = vsprintf('%s%s-%s-%s-%s-%s%s%s', str_split(bin2hex(openssl_random_pseudo_bytes(16)), 4));
}
...
$thp = getenv('TRUSTED_HOST_PATTERNS');
if(isset($thp)) {
  $settings['trusted_host_patterns'] = explode(',', $thp);
}
...
$databases['default']['default'] = [
  ...
  'host' => fetchEnv('MYSQL_HOST', '127.0.0.1'),
  'port' => fetchEnv('MYSQL_PORT', 3306),
  'driver' => 'mysql',
  'prefix' => '',
  'collation' => 'utf8mb4_general_ci',
];
